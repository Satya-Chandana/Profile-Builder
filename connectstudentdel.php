...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql1 = "DELETE FROM studentdetails WHERE username = '$username' AND email = '$email'";

    // Delete user details from facultysignupdetails table
    $sql2 = "DELETE FROM studentsignupdetails WHERE username = '$username' AND email = '$email'";

    // Execute the queries
    if (mysqli_query($con, $sql1) && mysqli_query($con, $sql2)) {
        // User details deleted successfully from both tables
        header("Location: submitdel.html");
        exit();
    } else {
